...
import \
    webbrowser  # to import all this packages first we have to install all this modules using cammanand "pip install xyz"
...
def wiki():
    ...
    results = wikipedia.summary(query, sentences=2,
                                auto_suggest=False)  # summary function in wekipedia module helps in providing short summary
    print(results)
    speak(results)
...
def weather():
    ...
    while (query == "none"):
        query = userCommand().lower()
    soup = BeautifulSoup(requests.get(f"https://www.google.com/search?q=weather+in+{query}").text,
                         "html.parser")  # webscrapping
...
def advice():
    result = requests.get(
        "https://api.adviceslip.com/advice").json()  # api used to get random advice from given address
...
def email():
    mail = Mailer(email="neethueditbucks@gmail.com",
                  password="zezycddxaorcrhfb")  # function helps in stroing address and password and processing it
